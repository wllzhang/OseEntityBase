...
1. 路线规划功能, 地图上点击几个点, 使用贝塞尔模型,线性模型等连线, 然后将轨迹绑定到这个entity上  (已完成,2025年10月30日)
    - 我们分几步实现
    - 先实现一个基于geoentity的gemotry点绘制的基类, 可以绘制点和点的顺序标签(1,2,3,4)
    - 将这些点归为同一组,可以在geoentitymanager实现, 即通用的对entity实现为一组.
    - geoentitymanager然后实现一个方法让组中point形成线性的连线. 即 点组(控制点) 和 连线方法 会唯一确定这个线
    - 然后将这一组点与到某个实体关联上. 
...
- **全面修复**: 修复了拖拽添加实体、实体选择、鼠标位置追踪三个场景的坐标精度问题
