...
1. 路线规划功能, 地图上点击几个点, 使用贝塞尔模型,线性模型等连线, 然后将轨迹绑定到这个entity上
2. 与慧慧进行页面集成
3. 航向角设置有问题
4. 拖拽添加是坐标不精确
5. 选中时可以根据当前range 调整选中阈值大小   (已修复,从state获取range , 然后动态实现 2025年10月30日) 
6. 选中后视角移动有问题, 不可以滚轮键调整了.  (已修复 2025年10月30日)
7. 删除实体时会崩溃  (已修复,使用延时删除,先标记,然后再qtimer里统一处理 2025年10月30日)
8. 规划结果转化为afsim文档
9. **信息层抽象**
   - 地图状态信息抽象
   - 实体状态信息抽象
